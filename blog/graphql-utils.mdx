...
topic: Backend
description: Jenyus engineers developed a package for various implementations of GraphQL to optimize relational data queries in high-performance APIs.
date: 2021-03-08
tags:
  - name: GraphQL
    color: "#DE33A6"
...
A couple of weeks ago I wrote about the Jenyus `graphql-utils` package on my [personal blog](https://ravianand.web.app/blog/graphql-utils), showing how we could leverage the GraphQL AST to optimize relational queries and avoid dataloaders entirely. Today we're going to look at how Jenyus uses the `nestjs-graphql-utils` package we built on top of it to create scalable and performant APIs such as the one used in [Recog](https://github.com/Jenyus-Org/recog-api/).
...
Once you have the boilerplate setup, you'll see the available relationships are already optimized using `nestjs-graphql-utils`. Today we're going to go over the implementation of the `Post.author` relationship, and a couple of alternative methods.
