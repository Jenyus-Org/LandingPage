...
The NestJS framework, which we will be going over today, solves all those things for you. We're going to see how we can use their CLI to setup a barebones REST and GraphQL API, and take a look at some of the features this framework offers.
...
In order for NestJS to become aware of this newly created module, we need to add it to the `AppModule`. `imports` is where we would add `UserModule`, but since we used the CLI NestJS already does this for us.
...
The service pattern is a very popular and fundamental design pattern in software architecture. Instead of putting our business logic right in the REST controllers we put them in services, in order to maintain the single-responsibility principle (SRP) and maintain the separation of concerns (SOC).
...
Building REST APIs with NestJS controllers is an extremely intuitive process. Whereas in ExpressJS one would use routers to groupe route handlers, NestJS controllers let us define a root path, such as `/api/users`, and then create methods to handle individual routes. Just like ExpressJS it supports wildcard patterns to create a RESTful API.
...
By registering `AppService` in `AppModule` under the `providers`, NestJS is able to use our type-hint in the `AppController`'s controller to figure out what to inject.
...
Probably one of the most popular libraries in NodeJS is PassportJS. With over 1 million weekly downloads on [npm](https://www.npmjs.com/package/passport) it's the tried and tested authentication library Express-based web APIs and uses strategies to let you define your own methods of authentication.
